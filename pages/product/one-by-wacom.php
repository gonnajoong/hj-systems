<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel ="shortcut icon" href="../../images/favicon/favicon-16.ico" sizes="16x16">
      <link rel ="shortcut icon" href="../../images/favicon/favicon-32.ico" sizes="32x32">
      <link rel="shortcut icon" href="../../images/favicon/favicon-32.png">
      <link rel="shortcut icon" href="../../images/favicon/favicon-96.png">
      <link rel="shortcut icon" href="../../images/favicon/favicon-152.png">

      <meta property="og:type" content="website"/>
<meta property="og:url" content="http://hjsystems.co.kr"/>
<meta property="og:image" content="/images/thumbnail.jpg"/>
<meta property="og:title" content="에이치제이시스템즈 | HJ SYSTEMS"/>
<meta property="og:description" content="안녕하세요 와콤 타블렛 총판 에이치제이시스템즈 입니다."/>

      <meta charset="UTF-8" />
      <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      <script src="../../js/libs/jquery-3.5.1.min.js"></script>
      <script src="https://unpkg.com/aos@2.3.1/dist/aos.js"></script>
      <script src="../../js/libs/slick.min.js"></script>

      <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@100;300;400;500;700;900&display=swap" rel="stylesheet">
      <link rel="stylesheet" href="../../assets/index.css" />

    <title>HJ SYSTEMS</title>
</head>
<body>
    <?php @include("../../layouts/header.php");?>
    <script type="text/javascript" src="../../pages/layouts/header.js"></script>
    <section class="hj-product-detail-wrap">
        <article class="hj-product-detail">
            <section class="hj-product-content-wrap">
                <div class="hj-product-image">
                    <ul id="hjPageSlideButton" class="hj-page-slide-button slick-dots">
                        <li data-snav='1' class="hj-active"><button>원바이와콤</button></li>
                        <li data-snav='2'><button>상세 정보</button></li>
                        <li data-snav='3'><button>제품 크기비교</button></li>
                        <li data-snav='4'><button>제품 사양</button></li>
                        <!-- <li><button>분배기 or 젠더</button></li>
                        <li><button>구매 및 견적 상담</button></li> -->
                    </ul>
                    <ul id="hjPageInSlideWrap" class="hj-page-in-slide-wrap product-wrap one-by-wacom-wrap">
                        <li data-page='1' class="hj-page-detail-image one-by-wacom hj-show"></li>
                        <li data-page='2' class="hj-page-detail-image one-by-wacom-button-01">
                            <img src="../../images/products/one-by-wacom/OneByWacom_소형중형통합_Jan2019.jpg" alt="와콤원">
                        </li>
                        <li data-page='3' class="hj-page-detail-image one-by-wacom-button-02">
                            <img src="../../images/products/one-by-wacom/One_by_wacom.jpg" alt="크기 비교이미지">
                        </li>
                        <li data-page='4' class="hj-page-detail-image one-by-wacom-button-03">
                            <table class="hj-product-spec-table">
                                <tr class="hj-table-main-header">
                                    <th>제품명</th>
                                    <th>One By Wacom (소형)</th>
                                    <th>One By Wacom (중형)</th>
                                </tr>
                                <tr>
                                    <th>모델명</th>
                                    <td>CTL-472/K0-CX</td>
                                    <td>CTL-672/K0-CX</td>
                                </tr>
                                <tr>
                                    <th>크기</th>
                                    <td>210 x 146 x 8.7mm (W x D x H)</td>
                                    <td>277 x 189 x 8.7mm (W x D x H)</td>
                                </tr>
                                <tr>
                                    <th>무게</th>
                                    <td>250g</td>
                                    <td>432g</td>
                                </tr>
                                <tr>
                                    <th>작업 영역</th>
                                    <td>152 x 95mm (W x D)</td>
                                    <td>216 x 135mm (W x D)</td>
                                </tr>
                                <tr>
                                    <th>색상</th>
                                    <td colspan="2">Black & Red</td>
                                </tr>
                                <tr>
                                    <th>보증기간</th>
                                    <td colspan="2">1년</td>
                                </tr>
                                <tr>
                                    <th>USB 케이블</th>
                                    <td colspan="2">
                                        Windows® 7 or later, OS X 10. 10 or later, standard USB<br>
                                        Type-A 포트, 드라이버 다운로드를 위한 인터넷 연결 필수
                                    </td>
                                </tr>
                                <tr>
                                    <th>멀티 터치</th>
                                    <td colspan="2">지원 안 함</td>
                                </tr>
                                <tr>
                                    <th>익스프레키</th>
                                    <td colspan="2">없음</td>
                                </tr>
                                <tr>
                                    <th>와콤 무선 키트</th>
                                    <td colspan="2">지원 안 함</td>
                                </tr>
                                <tr>
                                    <th>펜</th>
                                    <td colspan="2">무건전지 필압 감지 펜</td>
                                </tr>
                                <tr>
                                    <th>펜 압력 감지 레벨</th>
                                    <td colspan="2">2,048</td>
                                </tr>
                                <tr>
                                    <th>지우개 기능</th>
                                    <td colspan="2">없음</td>
                                </tr>
                                <tr>
                                    <th>구성품</th>
                                    <td colspan="2">
                                        펜 타블릿 소형 또는 중형, 펜, USB 케이블(1m), 교체용 표준펜심 (3개),<br>
                                        펜심 제거기, 퀵 스타트 가이드
                                    </td>
                                </tr>
                                <tr>
                                    <th>액세서리 (옵션)</th>
                                    <td colspan="2">펜 (LP190K), 표준 펜심(ACK20001)</td>
                                </tr>
                            </table>
                            <table class="hj-m-table">
                                <tr class="hj-table-main-header">
                                    <th colspan="2">One By Wacom (소형)</th>
                                </tr>
                                <tr>
                                    <th>모델명</th>
                                    <td>CTL-472/K0-CX</td>
                                </tr>
                                <tr>
                                    <th>크기</th>
                                    <td>210 x 146 x 8.7mm (W x D x H)</td>
                                </tr>
                                <tr>
                                    <th>무게</th>
                                    <td>250g</td>
                                </tr>
                                <tr>
                                    <th>작업 영역</th>
                                    <td>152 x 95mm (W x D)</td>
                                </tr>
                                <tr>
                                    <th>색상</th>
                                    <td>Black & Red</td>
                                </tr>
                                <tr>
                                    <th>보증기간</th>
                                    <td>1년</td>
                                </tr>
                                <tr>
                                    <th>USB 케이블</th>
                                    <td>
                                        Windows® 7 or later, OS X 10. 10 or later, standard USB<br>
                                        Type-A 포트, 드라이버 다운로드를 위한 인터넷 연결 필수
                                    </td>
                                </tr>
                                <tr>
                                    <th>멀티 터치</th>
                                    <td>지원 안 함</td>
                                </tr>
                                <tr>
                                    <th>익스프레키</th>
                                    <td>없음</td>
                                </tr>
                                <tr>
                                    <th>와콤 무선 키트</th>
                                    <td>지원 안 함</td>
                                </tr>
                                <tr>
                                    <th>펜</th>
                                    <td>무건전지 필압 감지 펜</td>
                                </tr>
                                <tr>
                                    <th>펜 압력 감지 레벨</th>
                                    <td>2,048</td>
                                </tr>
                                <tr>
                                    <th>지우개 기능</th>
                                    <td>없음</td>
                                </tr>
                                <tr>
                                    <th>구성품</th>
                                    <td>
                                        펜 타블릿 소형 또는 중형, 펜, USB 케이블(1m), 교체용 표준펜심 (3개),<br>
                                        펜심 제거기, 퀵 스타트 가이드
                                    </td>
                                </tr>
                                <tr>
                                    <th>액세서리 (옵션)</th>
                                    <td>펜 (LP190K), 표준 펜심(ACK20001)</td>
                                </tr>
                            </table>
                            <table class="hj-m-table">
                                <tr class="hj-table-main-header">
                                    <th colspan="2" >One By Wacom (중형)</th>
                                </tr>
                                <tr>
                                    <th>모델명</th>
                                    <td>CTL-672/K0-CX</td>
                                </tr>
                                <tr>
                                    <th>크기</th>
                                    <td>277 x 189 x 8.7mm (W x D x H)</td>
                                </tr>
                                <tr>
                                    <th>무게</th>
                                    <td>432g</td>
                                </tr>
                                <tr>
                                    <th>작업 영역</th>
                                    <td>216 x 135mm (W x D)</td>
                                </tr>
                                <tr>
                                    <th>색상</th>
                                    <td>Black & Red</td>
                                </tr>
                                <tr>
                                    <th>보증기간</th>
                                    <td>1년</td>
                                </tr>
                                <tr>
                                    <th>USB 케이블</th>
                                    <td>
                                        Windows® 7 or later, OS X 10. 10 or later, standard USB<br>
                                        Type-A 포트, 드라이버 다운로드를 위한 인터넷 연결 필수
                                    </td>
                                </tr>
                                <tr>
                                    <th>멀티 터치</th>
                                    <td>지원 안 함</td>
                                </tr>
                                <tr>
                                    <th>익스프레키</th>
                                    <td>없음</td>
                                </tr>
                                <tr>
                                    <th>와콤 무선 키트</th>
                                    <td>지원 안 함</td>
                                </tr>
                                <tr>
                                    <th>펜</th>
                                    <td>무건전지 필압 감지 펜</td>
                                </tr>
                                <tr>
                                    <th>펜 압력 감지 레벨</th>
                                    <td>2,048</td>
                                </tr>
                                <tr>
                                    <th>지우개 기능</th>
                                    <td>없음</td>
                                </tr>
                                <tr>
                                    <th>구성품</th>
                                    <td>
                                        펜 타블릿 소형 또는 중형, 펜, USB 케이블(1m), 교체용 표준펜심 (3개),<br>
                                        펜심 제거기, 퀵 스타트 가이드
                                    </td>
                                </tr>
                                <tr>
                                    <th>액세서리 (옵션)</th>
                                    <td>펜 (LP190K), 표준 펜심(ACK20001)</td>
                                </tr>
                            </table>
                        </li>
                        <!-- <li class="hj-page-detail-image dtc-133-button-04"></li>
                        <li class="hj-page-detail-image dtc-133-button-05"></li> -->
                    </ul>
                    <button class="hj-move-top">
                        <span>TOP</span>
                    </button>
                </div>
            </section>
        </article>
    </section>
    <?php @include("../../layouts/footer.php");?>
    <script type="text/javascript" src="/pages/main/js/index.js"></script>
    <script type="text/javascript" src="/pages/js/index.js"></script>
</body>
</html>